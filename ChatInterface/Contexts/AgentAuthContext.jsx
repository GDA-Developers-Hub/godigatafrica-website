...
export const AgentAuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    ...
    try {
      console.log("AgentAuthContext: Attempting login with email:", email);
      console.log("AgentAuthContext: Using API endpoint:", `${API_BASE_URL}/agent-auth/login`);
      ...
      console.log("AgentAuthContext: Login response status:", res.status);
      console.log("AgentAuthContext: Login response structure:", {
      ...
      if (res.data.success) {
        ...
        console.log("AgentAuthContext: Token stored in localStorage");
        
        // The agent-auth/login endpoint returns user data with agentId and agentName 
        // according to backend implementation
        const userData = res.data.user;
        console.log("AgentAuthContext: User data structure received:", 
          Object.keys(userData).reduce((acc, key) => {
            acc[key] = key.includes('password') ? '[REDACTED]' : (userData[key] ? typeof userData[key] : 'null');
            return acc;
          }, {})
        );
        
        localStorage.setItem("agentInfo", JSON.stringify(userData));
        console.log("AgentAuthContext: Agent info stored in localStorage");
        
        // Set agent state
        setAgent(userData);
        console.log("AgentAuthContext: Agent state updated");
        
        return { success: true };
      } else {
        console.log("AgentAuthContext: Login failed with message:", res.data.message);
      ...
      console.error("AgentAuthContext: Login error:", err);
      console.log("AgentAuthContext: Error response:", {
      ...
      console.log("AgentAuthContext: Final error message:", errorMessage);
