...
import { API_BASE_URL } from "../ChatBaseUrl";
...
const AgentDashboard = () => {
 ...
 const [formData, setFormData] = useState({
  ...
  useEffect(() => {
    // Pre-fetch admins data so it's ready
  ...
  useEffect(() => {
    const fetchAgents = async () => {
      ...
      setError(null);
      try {
        console.log("AgentDashboard: Fetching agents data");
        // Get admin token
        ...
        // Use admin-agent endpoint instead of agent endpoint
        console.log("AgentDashboard: Making API request to:", `${API_BASE_URL}/admin-agent/agents`);
        // Make authenticated request to fetch agents
        const response = await axios.get(`${API_BASE_URL}/admin-agent/agents`, config);
        
        console.log("AgentDashboard: API response status:", response.status);
        console.log("AgentDashboard: API response success:", response.data.success);
        
        if (response.data.success) {
          console.log("AgentDashboard: Agents fetched successfully, count:", response.data.agents.length);
          ...
          console.log("AgentDashboard: Agent status counts:", {
            online: activeCount,
            busy: busyCount,
          ...
          console.error("AgentDashboard: Failed to fetch agents:", response.data.message);
          setError(response.data.message || "Failed to fetch agents");
        }
      } catch (err) {
        console.error("AgentDashboard: Error fetching agents:", err);
        ...
        if (err.response?.status === 401) {
          console.log("AgentDashboard: Authentication error, redirecting to login");
          // Clear storage
    ...
    // Only fetch if we have admin info
    if (adminInfo) {
      fetchAgents();
    }
  }, [refreshTrigger, adminInfo, navigate]);
  ...
  const fetchAdmins = async () => {
    try {
      ...
      console.log("AgentDashboard: Making API request to:", `${API_BASE_URL}/admin`);
      // Fetch admins
